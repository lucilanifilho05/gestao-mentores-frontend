...
export function ClassDialog({ open, mode, target, courses, onClose, onSaved }: Props): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    resetCourse({ cursoId: defaultCourseId });
    mutation.reset();
  ...
  async function submit(data: ClassFormData, courseId?: string): Promise<void> {
    ...
    else if (mode === 'edit' && target) saved = await updateMutation.mutateAsync({ id: target.id, payload: data });
  ...
  const form = (
    <div className="space-y-5 px-6 py-6">
      ...
      {mode === 'create' ? <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-800">Curso</span><select className={`gm-input ${courseErrors.cursoId ? 'gm-input-error' : ''}`} {...registerCourse('cursoId', { required: true })}><option value="">Selecione um curso</option>{courses.map((course) => <option key={course.id} value={course.id}>{course.nome}</option>)}</select>{courseErrors.cursoId ? <p className="mt-1.5 text-sm text-red-700">Selecione o curso da turma.</p> : null}</label> : <div className="rounded-xl bg-slate-50 px-4 py-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Curso</p><p className="mt-1 font-semibold text-slate-900">{target?.curso.nome}</p></div>}
  ...
  const submitForm = mode === 'create' ? handleCourseSubmit((course) => handleSubmit((data) => submit(data, course.cursoId))()) : handleSubmit((data) => submit(data));
