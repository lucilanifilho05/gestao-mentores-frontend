import { CalendarClock, CheckCircle2, Clock3, MessageSquare, Pencil, Play, UserRound } from "lucide-react";
    ...
    <article className={`w-full overflow-hidden rounded-xl border bg-white shadow-sm transition hover:border-blue-300 hover:shadow-md ${task.possuiComentarioNaoLido ? "border-amber-400 ring-2 ring-amber-100" : "gm-border"}`}>
      <button type="button" onClick={onClick} className="w-full p-4 text-left">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t gm-border pt-3">
          ...
          <span className={`inline-flex shrink-0 items-center gap-1.5 text-xs font-bold ${overdue ? "text-red-700" : task.status === "concluida" ? "text-emerald-700" : task.status === "em_andamento" ? "text-blue-700" : "text-slate-600"}`}>{task.status === "concluida" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <CalendarClock className="h-3.5 w-3.5" />}{date}</span>
        </div>
      </button>
      {hasActions ? (
