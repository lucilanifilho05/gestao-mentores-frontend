import { CalendarClock, CheckCircle2, Clock3, Pencil, UserRound } from "lucide-react";
...
export function TaskCard({
  ...
  return (
    <article className="w-full overflow-hidden rounded-xl border gm-border bg-white shadow-sm transition hover:border-blue-300 hover:shadow-md">
      <button type="button" onClick={onClick} className="w-full p-4 text-left">
        <div className="flex items-start justify-between gap-3">
          ...
          {task.quantidadeReagendamentos > 0 ? <span className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-amber-700"><Clock3 className="h-3.5 w-3.5" />{task.quantidadeReagendamentos}</span> : null}
        </div>
